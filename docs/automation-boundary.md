...
API-only 同步是另一条独立链路：它在后端服务中调用微信小店官方开放 API，同步业务数据到标准表，不依赖 Playwright、浏览器 profile、扫码登录或后台导出文件。
...
- Codex、AI、MCP 不直接调用微信小店开放 API；真实 API 的 access token、authorizer access token、AppSecret、回调验签、接口额度和重试逻辑只允许后端服务处理。
- API-only 同步不模拟后台导出按钮，不请求未授权接口，不生成“后台原样导出文件”。需要文件时，只能由本系统基于已入库标准表生成 CSV/XLSX/报告。
...
- API 同步框架包含 `mock_wechat` 离线 connector 和 `wechat_api` 真实 connector。离线 connector 用于无网络回归；真实 connector 由服务端用商家自研 token 或 ISV 授权后的 `authorizer_access_token` 调用官方业务查询 API。
- `api_pull` 原始响应先保存到私有归档目录，并在 `sync_runs`、`sync_run_items`、`raw_api_responses` 中写入索引；商品、订单、售后退款和资金流水会通过专门 mapper 写入标准业务表，罗盘/经营数据在字段能匹配时写入分析表，同步完成后可复用现有指标和报告链路。
