...
FIELD_SPECS: dict[str, tuple[FieldSpec, ...]] = {
    ...
    "products": (
        FieldSpec("product_id", ("商品ID", "商品编号", "商品id", "商品编码", "商品货号", "商品SPU", "product_id", "spu_id", "goods_id", "goods_no")),
        FieldSpec("product_name", ("商品名称", "商品名", "商品标题", "标题", "product_name", "goods_name", "item_name"), required=True),
        FieldSpec("sku_id", ("SKU ID", "规格ID", "sku_id")),
        FieldSpec("sku_name", ("规格", "规格名称", "SKU", "sku_name", "sku")),
        FieldSpec("category", ("类目", "分类", "商品类目", "店铺类目", "平台类目", "category")),
        FieldSpec("status", ("商品状态", "上下架状态", "销售状态", "状态", "status")),
        FieldSpec("price", ("价格", "售价", "销售价", "商品价格", "现价", "最低价", "price"), kind="amount"),
        FieldSpec("stock", ("库存", "可售库存", "库存数量", "总库存", "剩余库存", "stock"), kind="amount"),
    ),
    "product_skus": (
        ...
        FieldSpec("sku_id", ("SKU ID", "规格ID", "sku_id", "sku编号", "SKU编号", "规格编码", "规格货号"), required=True),
        FieldSpec("sku_name", ("规格", "规格名称", "SKU", "sku_name", "sku", "规格值", "规格组合")),
        FieldSpec("category", ("类目", "分类", "商品类目", "店铺类目", "平台类目", "category")),
        FieldSpec("status", ("商品状态", "SKU状态", "上下架状态", "销售状态", "状态", "status")),
        FieldSpec("sku_price", ("SKU价格", "规格价格", "售价", "销售价", "价格", "现价", "sku_price", "price"), kind="amount"),
        FieldSpec("stock", ("库存", "可售库存", "库存数量", "总库存", "剩余库存", "stock"), kind="amount"),
        FieldSpec("barcode", ("条形码", "商品条码", "商家编码", "货号", "barcode")),
    ),
    "refunds": (
        FieldSpec("refund_id", ("售后单号", "退款单号", "退货单号", "售后编号", "退款编号", "售后ID", "退款ID", "aftersale_id", "after_sale_id", "afterSaleId", "refund_id")),
        FieldSpec("order_id", ("订单号", "订单编号", "订单ID", "交易单号", "order_id", "order_no"), required=True),
        FieldSpec("product_id", ("商品ID", "商品编号", "商品id", "product_id", "spu_id", "goods_id")),
        FieldSpec("product_name", ("商品名称", "商品名", "商品标题", "商品信息", "商品", "标题", "product_name", "goods_name", "item_name")),
        FieldSpec("sku_id", ("SKU ID", "规格ID", "sku_id")),
        FieldSpec("sku_name", ("规格", "规格名称", "SKU", "sku_name", "sku")),
        FieldSpec("refund_status", ("售后状态", "退款状态", "退货状态", "售后进度", "处理状态", "状态", "refund_status", "status")),
        FieldSpec("refund_amount", ("退款金额", "退款/退货金额", "实际退款金额", "实际退还金额", "实退金额", "已退款金额", "订单退款金额", "商品退款金额", "售后金额", "退还金额", "refund_amount"), kind="amount", required=True),
        FieldSpec("refund_created_at", ("申请时间", "售后申请时间", "申请售后时间", "退款申请时间", "退货申请时间", "创建时间", "refund_created_at", "created_at"), kind="datetime"),
        FieldSpec("refund_completed_at", ("退款完成时间", "退款成功时间", "售后完成时间", "退货完成时间", "完成时间", "处理完成时间", "refund_completed_at", "completed_at"), kind="datetime"),
        FieldSpec("reason", ("退款原因", "售后原因", "退货原因", "申请原因", "原因说明", "原因", "reason")),
    ),
    "reviews": (
        FieldSpec("shop_name_snapshot", ("店铺名称", "店铺名", "小店名称", "shop_name", "shop")),
        FieldSpec("review_id", ("评价ID", "评论ID", "评价编号", "评论编号", "review_id", "comment_id")),
        ...
        FieldSpec("sku_name", ("规格", "规格名称", "SKU", "sku_name", "sku")),
        FieldSpec("buyer_id", ("买家ID", "用户ID", "客户ID", "买家账号", "openid", "buyer_id", "customer_id")),
        FieldSpec("rating", ("评分", "星级", "评价分", "评价等级", "评价星级", "商品评分", "服务评分", "rating", "score"), kind="amount"),
        FieldSpec("review_content", ("评价内容", "评论内容", "评价详情", "评论详情", "买家评价", "用户评价", "评价文本", "评论文本", "评价", "评论", "review_content", "content"), required=True),
        FieldSpec("review_created_at", ("评价时间", "评论时间", "评价创建时间", "评论创建时间", "创建时间", "review_created_at", "comment_time"), kind="datetime"),
        FieldSpec("reply_content", ("商家回复", "商家回复内容", "回复内容", "reply_content", "seller_reply")),
    ...
    "fund_flows": (
        FieldSpec("shop_name_snapshot", ("店铺名称", "店铺名", "小店名称", "shop_name", "shop")),
        FieldSpec("flow_id", ("流水号", "资金流水号", "账单流水号", "交易流水号", "微信支付流水号", "账单号", "商户单号", "微信支付单号", "flow_id", "transaction_id")),
        FieldSpec("flow_date", ("入账时间", "记账时间", "发生时间", "交易时间", "流水时间", "结算时间", "账单日期", "创建时间", "完成时间", "日期", "flow_date", "transaction_time"), kind="datetime", required=True),
        FieldSpec("flow_type", ("收支类型", "流水类型", "资金类型", "账务类型", "交易类型", "类型", "flow_type")),
        FieldSpec("biz_type", ("业务类型", "业务场景", "账务类型", "账单类型", "交易类型", "biz_type")),
        FieldSpec("order_id", ("订单号", "订单编号", "订单ID", "交易单号", "order_id", "order_no")),
        FieldSpec("amount", ("金额", "收支金额", "收入金额", "支出金额", "入账金额", "变动金额", "交易金额", "结算金额", "实收金额", "退款金额", "amount"), kind="amount", required=True),
        FieldSpec("currency", ("币种", "currency")),
        FieldSpec("direction", ("收支方向", "方向", "收入支出", "收入/支出", "direction")),
        FieldSpec("balance", ("账户余额", "当前余额", "余额", "可用余额", "balance"), kind="amount"),
    ...
    "ad_spend": (
        ...
        FieldSpec("campaign_id", ("计划ID", "广告计划ID", "推广计划ID", "campaign_id", "plan_id")),
        FieldSpec("campaign_name", ("计划名称", "广告计划", "广告计划名称", "推广计划", "推广计划名称", "campaign_name", "plan_name")),
        FieldSpec("ad_group_id", ("单元ID", "广告组ID", "推广单元ID", "ad_group_id", "unit_id")),
        FieldSpec("ad_group_name", ("单元名称", "广告组名称", "推广单元", "推广单元名称", "ad_group_name", "unit_name")),
        FieldSpec("product_id", ("商品ID", "商品编号", "商品id", "product_id", "spu_id", "goods_id")),
        FieldSpec("product_name", ("商品名称", "商品名", "标题", "商品标题", "product_name", "goods_name", "item_name")),
        FieldSpec("impressions", ("展现量", "展现次数", "曝光量", "展示量", "曝光次数", "impressions", "show"), kind="amount"),
        FieldSpec("clicks", ("点击量", "点击数", "点击次数", "clicks", "click"), kind="amount"),
        FieldSpec("spend_amount", ("消耗", "花费", "花费金额", "广告花费", "投放消耗", "推广消耗", "现金消耗", "总消耗", "spend_amount", "cost"), kind="amount", required=True),
        FieldSpec("payment_amount", ("成交金额", "支付金额", "转化金额", "投放成交金额", "广告成交金额", "成交GMV", "GMV", "gmv", "payment_amount"), kind="amount"),
        FieldSpec("orders", ("成交订单数", "支付订单数", "成交订单量", "订单数", "转化订单数", "orders"), kind="amount"),
...
HEURISTIC_TOKENS: dict[str, tuple[tuple[str, ...], ...]] = {
    ...
    "refund_id": (("售后", "单号"), ("退款", "单号"), ("售后", "编号"), ("refund", "id"), ("after", "sale", "id")),
    "refund_status": (("售后", "状态"), ("退款", "状态"), ("refund", "status")),
    "refund_created_at": (("申请", "时间"), ("申请", "售后", "时间"), ("created", "at")),
    "refund_completed_at": (("退款", "成功", "时间"), ("完成", "时间"), ("completed", "at")),
...
FILE_HINTS: dict[str, tuple[str, ...]] = {
    ...
    "products": ("product", "goods", "商品", "商品列表", "导出商品", "product_list"),
    "product_skus": ("sku", "规格", "商品规格", "库存", "商品列表", "导出商品", "product_list"),
    "refunds": ("refund", "after", "售后", "退款", "退货", "售后单", "售后订单", "退款订单", "维权"),
    "reviews": ("review", "comment", "评价", "评论", "商品评价", "评价管理", "买家评价", "评论管理"),
    ...
    "fund_flows": ("fund", "flow", "finance", "资金", "流水", "账单", "收支", "资金流水", "账户流水", "账务流水", "结算", "对账单", "账单明细"),
    "ad_spend": ("ad", "ads", "campaign", "广告", "投放", "消耗", "推广", "广告数据", "推广数据", "投放数据", "计划报表", "消耗明细"),
